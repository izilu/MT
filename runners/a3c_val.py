...
def a3c_val(
    ...
    while count < max_count:   #一类场景跑250次
        ...
        #np.savetxt("/data_sdd/datadrh/HOZ/visualization_files/out_save/episode_out/" + scene_type + str(count)+ ".txt", player.out_save.detach().cpu().numpy())

        spl, best_path_length = compute_spl(player, player_start_state)
        nsnpl, isNavigationPhase, isNavigationSuccess = compute_NSNPL(player)
        sne = compute_sne(player)
        # # sxz add
        record_actions = save_actions(actions, player.episode, spl, sne, scene_type, player_start_state, nsnpl, isNavigationPhase, isNavigationSuccess)
        ...
        bucketed_spl = get_bucketed_metrics(spl, sne,  best_path_length, player.success,action_value, nsnpl, isNavigationPhase, isNavigationSuccess)
