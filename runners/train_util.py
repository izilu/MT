...
def save_actions(actions, episode, spl, sne, scene_type, agent_start_state, nsnpl, isNavigationPhase, isNavigationSuccess):
    if spl>0:
        success = True
    else:
        success = False
    ...
    save_dict = {'scene': episode._env.scene_name,
                 ...
                 'sne': sne,
                 'nsnpl': nsnpl,
                 'isNavigationPhase': isNavigationPhase,
                 'isNavigationSuccess': isNavigationSuccess}
    return save_dict
...
def get_bucketed_metrics(spl, sne, best_path_length, success, action_value, nsnpl, isNavigationPhase, isNavigationSuccess):
    out = {}
    for i in [1, 5]:
        if best_path_length >= i:
            ...
            out["GreaterThan/{}/SNE".format(i)] = sne
            out["GreaterThan/{}/NSNPL * SSR".format(i)] = nsnpl 
            out["GreaterThan/{}/SSR".format(i)] = isNavigationPhase
            out["GreaterThan/{}/NSR * SSR".format(i)] = isNavigationSuccess 
    return out
...
def compute_sne(player):
    if not player.success:
        return 0
    else:
        return player.eps_len/(player.nav_len+1)   #+1是为了防止除以0

def compute_NSNPL(player):   #第一个返回值 min_navigation_length / navigation_length， 第二个返回值 是否算作是进入了navigation阶段
    if player.success and player.nav_len == 0:
        return 0, 0, 0

    if not player.success and player.nav_len != 0:
        return 0, 1, 0
    ...
    for obj_id in player.episode.task_data:
        try:
            _, best_nav_path_len, _ = player.environment.controller.shortest_path_to_target(
                player.search_nav_sate, obj_id, False
            )
            if best_nav_path_len < best:
                best = best_nav_path_len
        except:
            continue

    if best < float("inf"):
        return best / max(float(player.nav_len), best), 1, 1
