...
sys.path.append('/root/autodl-tmp/HOZ/models')
...
class Multi_Thinking_4T(torch.nn.Module):
    def __init__(self, args):
        ...
        pointwise_in_channels = 64 + self.num_cate + self.nav_embedding_dim + 8 + 8
        ...
        self.nav_all_dim = args.nav_all_dim
        self.obstacle_dim = args.obstacle_dim
        self.nav_length = args.nav_length

        self.linear_graph = nn.Sequential(
            nn.Linear(self.nav_dim + 2, 32),  
            nn.ReLU(),
            nn.Linear(32, 16), 
        )

        self.linear_graph_all = nn.Sequential(
            nn.Linear(self.nav_all_dim + 2, 16),  
            nn.ReLU(),
            nn.Linear(16, 8), 
        )

        self.linear_obstacle = nn.Sequential(
            nn.Linear(2, 4),
            nn.ReLU(),
            nn.Linear(4, 8),
        )
    ...
    def coord_centring_all(self, navgraph_all, coord):
        navgraph_all_copy = torch.zeros(navgraph_all.shape[0], 4 + 2).to(navgraph_all.device)
        nav_coord = navgraph_all[:,: 2]
        rotation = navgraph_all[:,2]
        horizon = navgraph_all[:,3]   

        # navgraph_all_copy[:,0] = navgraph_all[:,4]         
        navgraph_all_copy[:, : 2] = nav_coord - coord[:2] 
        
        # print("原坐标：", navgraph_all_copy[:,1 : 3])
        #将坐标表示表成极坐标 
        x = navgraph_all_copy[:,1] == 0
        x_direction = navgraph_all_copy[:,0] > 0 
        y_direction = navgraph_all_copy[:,1] > 0
        second_qua = (~x_direction) & y_direction
        third_qua = (~x_direction) & (~y_direction)
        neg_x = x & (~x_direction)

        thea = torch.atan(navgraph_all_copy[:,1]/navgraph_all_copy[:,0])
        thea[second_qua] += torch.tensor(math.pi)
        thea[third_qua] -= torch.tensor(math.pi)
        thea[neg_x] = torch.tensor(math.pi)
  
        length = torch.sqrt(torch.pow(navgraph_all_copy[:,0], 2) + torch.pow(navgraph_all_copy[:,1], 2))

        navgraph_all_copy[:,0] = thea
        navgraph_all_copy[:,1] = length
        ##################################
        # print("极坐标：", navgraph_all_copy[:,1 : 3])

        navgraph_all_copy[:,2] = torch.sin(rotation - coord[2])
        navgraph_all_copy[:,3] = torch.cos(rotation - coord[2])
        navgraph_all_copy[:,4] = torch.sin(horizon - coord[3])
        navgraph_all_copy[:,5] = torch.cos(horizon - coord[3])

        return navgraph_all_copy  #(x, y, sin(r), cos(r), sin(h), cos(h))
    
    def obstacle_centering(self, obstacle_graph, coord):
        obstacle_graph_copy = torch.zeros(obstacle_graph.shape[0], 2).to(obstacle_graph.device)   

        # navgraph_all_copy[:,0] = navgraph_all[:,4]         
        obstacle_graph_copy[:, : 2] = obstacle_graph - coord[:2] 
        
        # print("原坐标：", navgraph_all_copy[:,1 : 3])
        #将坐标表示表成极坐标 
        x = obstacle_graph_copy[:,1] == 0
        x_direction = obstacle_graph_copy[:,0] > 0 
        y_direction = obstacle_graph_copy[:,1] > 0
        second_qua = (~x_direction) & y_direction
        third_qua = (~x_direction) & (~y_direction)
        neg_x = x & (~x_direction)

        thea = torch.atan(obstacle_graph_copy[:,1]/obstacle_graph_copy[:,0])
        thea[second_qua] += torch.tensor(math.pi)
        thea[third_qua] -= torch.tensor(math.pi)  
        thea[neg_x] = torch.tensor(math.pi)
  
        length = torch.sqrt(torch.pow(obstacle_graph_copy[:,0], 2) + torch.pow(obstacle_graph_copy[:,1], 2))

        obstacle_graph_copy[:,0] = thea
        obstacle_graph_copy[:,1] = length
        ##################################
        return obstacle_graph_copy



    def get_obstacle(self, coord, start_coord):
        obstacle_node = torch.zeros(1,self.obstacle_dim)  #(x, y)
        if coord.rotation == 0:
            obstacle_node[:,0] = coord.x
            obstacle_node[:,1] = coord.z + self.grid_size
        elif coord.rotation == 90:
            obstacle_node[:,0] = coord.x + self.grid_size
            obstacle_node[:,1] = coord.z
        elif coord.rotation == 180:
            obstacle_node[:,0] = coord.x
            obstacle_node[:,1] = coord.z - self.grid_size
        elif coord.rotation == 270:
            obstacle_node[:,0] = coord.x - self.grid_size
            obstacle_node[:,1] = coord.z
        elif coord.rotation == 45:
            obstacle_node[:,1] = coord.z + self.grid_size
            obstacle_node[:,0] = coord.x + self.grid_size
        elif coord.rotation == 135:
            obstacle_node[:,1] = coord.z - self.grid_size
            obstacle_node[:,0] = coord.x + self.grid_size
        elif coord.rotation == 225:
            obstacle_node[:,1] = coord.z - self.grid_size
            obstacle_node[:,0] = coord.x - self.grid_size
        elif coord.rotation == 315:
            obstacle_node[:,1] = coord.z + self.grid_size
            obstacle_node[:,0] = coord.x - self.grid_size
        
        #相对于初始位置储存
        obstacle_node[:,0] = obstacle_node[:,0] - start_coord['x']
        obstacle_node[:,1] = obstacle_node[:,1] - start_coord['y']
        return obstacle_node

    def embedding(self, state, target, action_embedding_input, target_object, nav_graph, nav_graph_all, obstacle_graph, coord_origin, start_coord):
        ...
        coord, start_coord = self.get_coord(coord_origin, start_coord)
        ...
        nav_all_node  =  torch.zeros(1,self.nav_all_dim).to(target.device)
        nav_all_node[:, :] = coord
        
        #坐标支路输入
        if nav_graph_all.sum() == 0:
            nav_graph_all = nav_all_node
        else:
            last_coord = nav_graph_all[-1, : ]
            nav_graph_all = torch.cat((nav_graph_all, nav_all_node), dim = 0)

        #障碍物支路输入
            if last_coord.equal(coord):
                obstacle_node = self.get_obstacle(coord_origin, start_coord)  #get_obstacle从不能动的状态推断障碍物的位置
                obstacle_node = obstacle_node.to(target.device)
                if obstacle_graph.sum() == 0:     
                    obstacle_graph = obstacle_node
                else:          
                    obstacle_graph = torch.cat((obstacle_graph, obstacle_node), dim = 0)  #相对于初始坐标的障碍物位置
        ...
        #坐标支路编码
        if nav_graph_all.sum() == 0:
            nav_graph_all_mean = torch.zeros(1 ,8, 7, 7).to(target.device)
        else:
            nav_graph_all_center = self.coord_centring_all(nav_graph_all, coord)   #(n, 4) -> (n, 6)
            nav_graph_all_embeded = self.linear_graph_all(nav_graph_all_center)   #(n, 6) -> (n, 8)
            nav_graph_all_mean = torch.mean(nav_graph_all_embeded, dim = 0)
            nav_graph_all_mean = nav_graph_all_mean.view(1, 8, 1, 1).repeat(1, 1, 7, 7)

        #障碍物支路编码
        if obstacle_graph.sum() == 0: 
            obstacle_graph_mean = torch.zeros(1 , 8, 7, 7).to(target.device)
        else:
            obstacle_graph_center = self.obstacle_centering(obstacle_graph, coord)
            obstacle_graph_embeded = self.linear_obstacle(obstacle_graph_center) #(n, 2) -> (n, 8)
            obstacle_graph_mean = torch.mean(obstacle_graph_embeded, dim = 0)
            obstacle_graph_mean = obstacle_graph_mean.view(1, 8, 1, 1).repeat(1, 1, 7, 7)
        ...
        #Adaptive Fusion
        x = torch.cat((x, target_embedding, nav_graph_mean, nav_graph_all_mean, obstacle_graph_mean), dim=1)
        ...
        return out, image_embedding, action_at, nav_graph, nav_graph_all, obstacle_graph, start_coord
    ...
    def forward(self, model_input, model_options):
        ...
        nav_graph_all = model_input.nav_graph_all   
        obstacle_graph = model_input.obstacle_graph                                     
        ...
        x, image_embedding , action_at,  nav_graph, nav_graph_all, obstacle_graph, start_coord= self.embedding(state, target, action_probs, target_object, nav_graph, nav_graph_all, obstacle_graph, coord, start_coord)
        ...
        return ModelOutput(
            ...
            nav_graph = nav_graph,
            nav_graph_all = nav_graph_all,
            obstacle_graph = obstacle_graph,
