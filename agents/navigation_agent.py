...
class NavigationAgent(ThorAgent):
    ...
    def eval_at_state(self, model_options):
        ...
        model_input.nav_graph_all = self.nav_graph_all
        model_input.obstacle_graph = self.obstacle_graph
        ...
        model_input.out_save = self.out_save
    ...
    def reset_hidden(self):
        ...
        self.nav_graph_all = gpuify(
            torch.zeros((1, self.nav_all_dim)), self.gpu_id
        )
        self.obstacle_graph = gpuify(
            torch.zeros((1, self.obstacle_dim)), self.gpu_id
        )
        self.coord_memory = gpuify(
            torch.zeros((1, self.coord_memory_dim)), self.gpu_id
        )
        self.start_coord = {'x': -10, 'y': -10, 'rotation': -10, 'horizon': -10}
        self.out_save = gpuify(
            torch.zeros((1, 3136)), self.gpu_id
        )
