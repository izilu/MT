...
from datasets.constants import OBSTACLE_REWARD, SEARCH_REWARD, EXPLORATION_REWARD
        ...
        if self.environment.controller.state in self.scene_states:
            if action["action"] != DONE:
                if self.environment.last_action_success:
                    ...
                    reward = reward + EXPLORATION_REWARD
                else:
                    self.failed_action_count += 1
                    #obstacle_reward
                    reward = reward + OBSTACLE_REWARD
        else:
